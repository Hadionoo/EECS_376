...
//gets box_pose_wrt_world
    ...
    for (int imodel = 0; imodel < num_models; imodel++) {
        if (imodel != i_box) { //if here, have a model NOT the box
            ...
            //want to get part_pose_wrt_box here
            affine_box_pose_wrt_world = xformUtils_.transformPoseToEigenAffine3d(box_pose_wrt_world);
...
void BoxInspector::compute_shipment_poses_wrt_world(osrf_gear::Shipment shipment_wrt_box,
    ...
    ROS_WARN("Computing shipment poses wrt world");
geometry_msgs::PoseStamped shipment_part_pose_wrt_world;
geometry_msgs::Pose shipment_part_pose_wrt_box;
int numParts = shipment_wrt_box.products.size();
osrf_gear::Model model;
string type;

for(int i =0; i<numParts; i++){
    shipment_part_pose_wrt_box = shipment_wrt_box.products[i].pose;
    type = shipment_wrt_box.products[i].type;
    shipment_part_pose_wrt_world = compute_stPose(box_pose_wrt_world.pose,shipment_part_pose_wrt_box);

    model.pose=shipment_part_pose_wrt_world.pose;
    model.type = type;  
