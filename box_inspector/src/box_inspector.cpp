...
//convert logical camera to coordinates wrt the world frame
...
vector<osrf_gear::Model> modelsSeenMisplaced;
vector<osrf_gear::Model> modelsDesiredNotAccountedFor;

bool properlyPlaced;
vector<bool> accountedFor;
//initalize the accountedFor vector
for(int i =0; i<num_parts_desired;i++){
accountedFor.push_back(false);
}
//look at the models seen and see if it matches up with the position of the models we desire
for(int i = 0; i<num_parts_seen; i++){
	...
	if(!properlyPlaced){
		modelsSeenMisplaced.push_back(model_seen);
	}
...
for(int i =0; i<num_parts_desired;i++){
	if(!accountedFor[i]){
		modelsDesiredNotAccountedFor.push_back(desired_models_wrt_world[i]);
	}
}


//In this section you would find the misplaced parts that are missing from the order
}
